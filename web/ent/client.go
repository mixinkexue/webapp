...
// QueryAdopter queries the adopter edge of a Pet.
func (c *PetClient) QueryAdopter(pe *Pet) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := pe.ID
		step := sqlgraph.NewStep(
...
// QuerySent queries the sent edge of a User.
func (c *UserClient) QuerySent(u *User) *PetQuery {
...
// QueryAdopted queries the adopted edge of a User.
func (c *UserClient) QueryAdopted(u *User) *PetQuery {
	query := (&PetClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Pet, Token, User []ent.Hook
	}
	inters struct {
		Pet, Token, User []ent.Interceptor
	}
)
