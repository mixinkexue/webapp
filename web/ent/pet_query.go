...
// QueryAdopter chains the current query on the "adopter" edge.
func (pq *PetQuery) QueryAdopter() *UserQuery {
	query := (&UserClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(pet.Table, pet.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, pet.AdopterTable, pet.AdopterColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *PetQuery) Clone() *PetQuery {
	...
	return &PetQuery{
		...
		withAdopter: pq.withAdopter.Clone(),
...
// WithAdopter tells the query-builder to eager-load the nodes that are connected to
// the "adopter" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PetQuery) WithAdopter(opts ...func(*UserQuery)) *PetQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withAdopter = query
	return pq
}
...
func (pq *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if pq.withOwner != nil || pq.withAdopter != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withAdopter; query != nil {
		if err := pq.loadAdopter(ctx, query, nodes, nil,
			func(n *Pet, e *User) { n.Edges.Adopter = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PetQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_sent == nil {
			continue
		}
		fk := *nodes[i].user_sent
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_sent" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PetQuery) loadAdopter(ctx context.Context, query *UserQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Pet)
	for i := range nodes {
		if nodes[i].user_adopted == nil {
			continue
		}
		fk := *nodes[i].user_adopted
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_adopted" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
